...
const App = () => {
  ...
  const RoomScreen = () => {
    ...
    // 拖曳與選取狀態
    const [draggingId, setDraggingId] = useState(null);
    const [selectedId, setSelectedId] = useState(null); // 新增：目前被選取的物品
    const [sceneIndex, setSceneIndex] = useState(0); // 新增：場景切換
    const containerRef = useRef(null);

    // 定義 4 種場景樣式 (CSS Gradients)
    const SCENES = [
        { 
            name: "Cozy Bedroom", 
            wall: "bg-[#FFF8E1]", // 米色牆
            floor: "bg-[#D7CCC8]", // 木頭地板色
            pattern: "radial-gradient(#8D6E63 1px, transparent 1px)", // 地板紋路
            accent: "border-[#8D6E63]"
        },
        { 
            name: "Sunny Garden", 
            wall: "bg-[#B3E5FC]", // 藍天
            floor: "bg-[#C8E6C9]", // 草地
            pattern: "radial-gradient(#4CAF50 2px, transparent 2px)", // 草地紋路
            accent: "border-[#4CAF50]"
        },
        { 
            name: "Deep Space", 
            wall: "bg-[#1A237E]", // 深藍宇宙
            floor: "bg-[#311B92]", // 深紫地面
            pattern: "radial-gradient(#FFF 1px, transparent 1px)", // 星星
            accent: "border-[#7986CB]"
        },
        { 
            name: "Candy Land", 
            wall: "bg-[#F8BBD0]", // 粉紅牆
            floor: "bg-[#F0F4C3]", // 奶油地板
            pattern: "radial-gradient(#F48FB1 2px, transparent 2px)", // 糖果點點
    ...
    const handleStart = (e, id) => {
        // 阻止瀏覽器預設行為 (例如捲動)
        e.stopPropagation(); 
        // e.preventDefault(); // 在 React 18 有時會擋住 click，視情況開啟
        
        setSelectedId(id); // 設定為選取狀態
        setDraggingId(id);
    };

    const handleMove = (e) => {
        if (!draggingId || !containerRef.current) return;
        e.preventDefault(); // 拖曳時一定要擋住捲動
        ...
        // 計算中心點位置：讓物品中心跟隨手指
        // 假設物品大約 80x80px，所以減去 40
        let newX = clientX - containerRect.left - 40;
        let newY = clientY - containerRect.top - 40;
    ...
    return (
        <div className="h-[85vh] flex flex-col pb-20 relative">
             {/* 頂部工具列 */}
             <div className="flex justify-between items-center mb-2 px-2 z-10">
                 <div className="flex flex-col">
                    <h2 className="text-2xl font-black text-gray-700 leading-none">{p.name}'s Room</h2>
                    <span className="text-xs text-gray-400 font-bold">Tap items to select/delete</span>
                 </div>
                 
                 <div className="flex gap-2">
                    <button onClick={toggleScene} className="bg-white border-2 border-gray-200 shadow-sm px-3 py-1 rounded-full text-xs font-black flex items-center gap-1 active:scale-95 transition-transform">
                        <RefreshCw size={12}/> Theme
                    </button>
                    <button onClick={() => setView('shop')} className="bg-[#55C1DE] text-white px-3 py-1 rounded-full font-black text-sm flex items-center gap-1 shadow-md active:scale-95 transition-transform">
                        <Plus size={14}/> Shop
                    </button>
                 </div>
             </div>
             ...
             <div 
                ref={containerRef}
                className={`flex-1 rounded-[2rem] border-[6px] ${currentScene.accent} shadow-inner relative overflow-hidden touch-none select-none transition-colors duration-500`}
                // 在容器上監聽移動，確保滑鼠/手指移出物品本身時也能拖曳
                onPointerMove={handleMove}
                onPointerUp={handleEnd}
                onPointerLeave={handleEnd}
                onTouchMove={handleMove}
                onTouchEnd={handleEnd}
                // 背景設定：上層是牆壁，下層是地板 (使用 gradient 模擬)
                style={{
                    background: `linear-gradient(to bottom, transparent 60%, rgba(0,0,0,0.1) 60%), linear-gradient(to bottom, transparent 60%, ${currentScene.floor.replace('bg-[', '').replace(']', '')} 60%)`
                }}
             >
                {/* 牆壁層 (CSS Background) */}
                <div className={`absolute top-0 left-0 right-0 bottom-[40%] ${currentScene.wall} -z-20 transition-colors duration-500`}></div>
                {/* 地板層 (CSS Background) */}
                <div className={`absolute bottom-0 left-0 right-0 h-[40%] ${currentScene.floor} -z-20 transition-colors duration-500`}></div>
                {/* 地板紋路 */}
                <div className="absolute bottom-0 left-0 right-0 h-[40%] opacity-30 pointer-events-none -z-10" style={{ backgroundSize: '20px 20px', backgroundImage: currentScene.pattern }}></div>

                {/* 提示文字 */}
                {items.length === 0 && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400/50 pointer-events-none z-0">
                        <Home size={64} className="mb-2 opacity-50"/>
                        <span className="font-black text-2xl">Empty Room</span>
                        <span className="text-sm font-bold">Tap items below to add!</span>
                    </div>
                )}
                ...
                {items.map(item => {
                    ...
                    return (
                        <div
                            key={item.id}
                            // 這裡使用 Pointer Events (整合滑鼠與觸控)
                            onPointerDown={(e) => handleStart(e, item.id)}
                            
                            className={`absolute flex flex-col items-center justify-center select-none transition-transform
                                ${isDragging ? 'scale-125 z-50 cursor-grabbing' : 'cursor-grab z-10'}
                                ${isSelected ? 'z-40' : ''}
                            `}
                            style={{ 
                                left: item.x, 
                                ...
                                width: '80px', // 加大觸控範圍
                                height: '80px',
                                touchAction: 'none' // 關鍵：禁止瀏覽器處理此元素的觸控捲動
                            }}
                        >
                            {/* 物品本體 (Emoji) */}
                            <span className="text-[4rem] drop-shadow-xl filter">{product.emoji}</span>
                            
                            {/* 選取時顯示的外框與刪除鈕 */}
                            {isSelected && (
                                <>
                                    <div className="absolute inset-0 border-2 border-dashed border-white/50 rounded-xl animate-pulse pointer-events-none"></div>
                                    <button 
                                        onPointerDown={(e) => {
                                            e.stopPropagation(); // 防止觸發拖曳
                                            removeFromRoom(item.id);
                                        }}
                                        className="absolute -top-4 -right-4 bg-red-500 text-white rounded-full w-8 h-8 flex items-center justify-center shadow-lg border-2 border-white hover:scale-110 active:scale-95 transition-transform z-50"
                                    >
                                        <Trash2 size={16} />
                                    </button>
                                </>
             ...
             <div className="mt-4 z-20">
                <div className="bg-white/90 backdrop-blur rounded-[1.5rem] p-3 shadow-lg border-2 border-white overflow-x-auto">
                    <div className="flex gap-3 min-w-min px-2">
                        {myInventory.length === 0 ? (
                            <div className="text-gray-400 text-xs font-bold w-full text-center py-3 flex flex-col items-center gap-1">
                                <span>No items yet...</span>
                                <button onClick={() => setView('shop')} className="text-[#55C1DE] underline">Go to Shop</button>
                            </div>
                        ) : (
                            myInventory.map((item, idx) => (
                                <button 
                                    key={`${item.id}-${idx}`}
                                    onClick={() => addToRoom(item.id)}
                                    className="flex-shrink-0 w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-3xl shadow-sm border-2 border-gray-100 hover:border-[#55C1DE] hover:bg-[#E0F7FA] active:scale-95 transition-all"
                                >
                                    {item.emoji}
                                </button>
                            ))
                        ...
                        {myInventory.length > 0 && (
                            <div className="w-[1px] h-10 bg-gray-200 mx-1 self-center"></div>
                        )}
                        <button 
                            onClick={() => setView('shop')}
                            className="flex-shrink-0 w-16 h-16 bg-[#FFF9C4] rounded-2xl flex flex-col items-center justify-center text-[#FBC02D] font-black text-[10px] border-2 border-[#FFF59D] active:scale-95"
                        >
